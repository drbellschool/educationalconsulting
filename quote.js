...
function calculate(d){
  ...
  if(isDistrict(d))return {bookCall:true};
...
function renderEstimate(showMessage){
  ...
  document.getElementById('emailLink').href='mailto:adam@lonestarteacher.com?subject='+subject+'&body='+body;
...
document.getElementById('quoteForm').addEventListener('input',()=>renderEstimate(false));
document.getElementById('quoteForm').addEventListener('reset',()=>setTimeout(()=>renderEstimate(false),0));
document.getElementById('quoteForm').addEventListener('submit',async e=>{
  ...
  try{
    ...
    msg.className='notice';msg.style.display='block';msg.textContent='The estimate was calculated, but submission failed. Please email Adam.';
  }
});
